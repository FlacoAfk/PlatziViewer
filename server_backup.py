...
# Configuración
COURSES_PATH = r"D:\platzi-downloader"
...
DRIVE_ROOT_ID = "17kPqqPSheDtQ5S1HM6Qvvh2qJ7O3YADm"  # Hardcoded root

def scan_drive_classes(folder_id, module_name=""):
    """Escanea clases (clases/videos) en una carpeta de Drive."""
    ds = get_drive_service()
    if not ds: return []
    
    classes = []
    try:
        files = ds.list_files_with_metadata(folder_id)
        # Ordenar por nombre
        files = sorted_items([f['name'] for f in files])
        
        # Re-fetch metadata for sorted items? No, list_files returns metadata objects.
        # We need to sort the objects by name.
        files_map = {f['name']: f for f in ds.list_files_with_metadata(folder_id)}
        sorted_names = sorted_items(files_map.keys())
        
        # Group by class number
        class_groups = {}
        for name in sorted_names:
            if name.startswith(('desktop', '.', 'Thumbs')): continue
            
            parts = name.split('. ', 1)
            if len(parts) >= 2 and parts[0].isdigit():
                class_num = int(parts[0])
                if class_num not in class_groups:
                    class_groups[class_num] = {'files': []}
                class_groups[class_num]['files'].append(files_map[name])
        
        for class_num in sorted(class_groups.keys()):
            files_list = class_groups[class_num]['files']
            ...
            for f in files_list:
                ...
                if fname.endswith('.mp4') or mime.startswith('video/'):
                    video_file = f
                elif fname.endswith('_summary.html'):
                    summary_file = f
                elif fname.endswith('.vtt'):
                    vtt_file = f
                elif 'Lecturas recomendadas' in fname:
                    reading_file = f
                elif fname.endswith('.html') and not fname.endswith('_summary.html'):
                    html_file = f
            
            if video_file:
                name = video_file['name'].rsplit('.', 1)[0]
                name = name.split('. ', 1)[-1] if '. ' in name else name
            elif html_file:
                name = html_file['name'].rsplit('.', 1)[0]
                name = name.split('. ', 1)[-1] if '. ' in name else name
            else:
                continue
            ...
            classes.append({
                ...
                'hasHtml': html_file is not None and video_file is None,
                'files': {
                    'video': f"drive:{video_file['id']}" if video_file else None,
                    'summary': f"drive:{summary_file['id']}" if summary_file else None,
                    'subtitles': f"drive:{vtt_file['id']}" if vtt_file else None,
                    'reading': f"drive:{reading_file['id']}" if reading_file else None,
                    'html': f"drive:{html_file['id']}" if html_file else None
                }
            })
            
    except Exception as e:
        print(f"❌ Error scanning drive classes in {folder_id}: {e}")
        
    return classes

def scan_drive_modules(folder_id):
    """Escanea módulos dentro de un curso en Drive."""
    ds = get_drive_service()
    ...
    try:
        files = ds.list_files_with_metadata(folder_id)
        # Filter folders
        folders = [f for f in files if ds.is_folder(f)]
        folders_map = {f['name']: f for f in folders}
        sorted_names = sorted_items(folders_map.keys())
        
        for name in sorted_names:
            f = folders_map[name]
            if name.startswith('.'): continue
            
            classes = scan_drive_classes(f['id'], name)
        ...
        print(f"❌ Error scanning drive modules in {folder_id}: {e}")
        
    return modules, has_presentation

def load_course_metadata():
    """Loads metadata to map course folders to categories/routes."""
    meta_path = os.path.join(VIEWER_PATH, 'server_metadata.json')
    mapping = {} # "Course Name" -> {"categoryId": ..., "routeId": ...}
            ...
            with open(meta_path, 'r', encoding='utf-8') as f:
                ...
                for cat in data.get('categories', []):
                    ...
                    for route in cat.get('routes', []):
                        ...
                        # Check courses inside route
                        for course in route.get('courses', []):
                            c_name = course.get('name') # This is the display name
                            c_folder = course.get('folder') # This matches the folder name in Drive?
                            if c_folder:
                                mapping[c_folder] = {'cat': cat, 'route': route}
        except Exception as e:
            print(f"⚠️ Error loading metadata: {e}")
    return mapping

def scan_drive_structure(root_id):
    """Escanea estructura completa desde Drive (Mapeo a Categorías)."""
    ds = get_drive_service()
    if not ds: return {'categories': [], 'stats': {}}
    
    print(f"☁️ Escaneando estructura desde Google Drive ({root_id})...")
    
    # Load metadata mapping
    metadata_map = load_course_metadata()
    
    # Categories dict to build structure
    built_categories = {}
    
    # Fallback category for unknown courses
    unknown_cat_id = 'downloads'
    built_categories[unknown_cat_id] = {
        'info': CATEGORIES['Courses'],
        'routes': {'drive-root': {'info': {'id': 'drive-root', 'name': 'Otros Cursos', 'isCourse': False}, 'courses': []}}
    }

    # Thread-safe lock for building categories
    cat_lock = threading.Lock()
    
    try:
        root_items = ds.list_files_with_metadata(root_id)
        folders = [f for f in root_items if ds.is_folder(f)]
        folders_map = {f['name']: f for f in folders}
        sorted_names = sorted_items(folders_map.keys())
        
        print(f"📂 Encontradas {len(folders)} carpetas raíz. Procesando en paralelo...")
        ...
        def process_folder(name):
            ...
            # Identify if this folder corresponds to a known course
            ...
            if meta:
                ...
                # Scan modules (expensive operation)
                modules, has_presentation = scan_drive_modules(f['id'])
                
                with cat_lock:
                    # Init category if not exists
                    if cat['id'] not in built_categories:
                        built_categories[cat['id']] = {
                            'info': cat,
                            'routes': {}
                        }
                    
                    # Init route if not exists
                    if route['id'] not in built_categories[cat['id']]['routes']:
                        built_categories[cat['id']]['routes'][route['id']] = {
                            'info': route,
                            'courses': []
                        }
                    
                    built_categories[cat['id']]['routes'][route['id']]['courses'].append({
                        'name': name,
                        'folderName': name,
                        'modules': modules,
                        'moduleCount': len(modules),
                ...
                # Unknown course -> Add to "Downloads" / "Otros"
                is_course = 'curso' in name.lower() or 'taller' in name.lower()
                
                if is_course:
                    modules, has_presentation = scan_drive_modules(f['id'])
                    
                    with cat_lock:
                        built_categories[unknown_cat_id]['routes']['drive-root']['courses'].append({
                            'name': name,
                            'folderName': name,
                            'modules': modules,
                            'moduleCount': len(modules),
        ...
        # Max workers? Drive API limit is ~10 queries/sec/user? No, it's higher.
        # But we don't want to hit 403 Rate Limit.
        # Let's try 10 workers.
        with ThreadPoolExecutor(max_workers=10) as executor:
            futures = [executor.submit(process_folder, name) for name in sorted_names]
            ...
            for future in as_completed(futures):
                completed += 1
                if completed % 5 == 0:
                    print(f"⏳ Progreso: {completed}/{total} carpetas procesadas...")
                try:
                    future.result()
                except Exception as exc:
                    print(f"❌ Error en worker: {exc}")
    ...
    # Convert built structure to final list
    ...
    # Ensure all original categories exist even if empty? No, only show what we have.
    # But for consistent UI order, maybe we should iterate server_metadata categories order?
    
    # Let's just output what we found.
    for cat_id, cat_data in built_categories.items():
        routes_list = []
        for route_id, route_data in cat_data['routes'].items():
            if not route_data['courses'] and not route_data['info'].get('isCourse'): continue
            
            r_info = route_data['info']
            routes_list.append({
                'id': r_info.get('id', route_id),
                'name': r_info.get('name', 'Sin nombre'),
                'folderName': r_info.get('folderName', ''),
                'isCourse': False, # Structure implies courses inside routes
                'courses': route_data['courses'],
                'courseCount': len(route_data['courses'])
            })
            
        if not routes_list: continue
            
        c_info = cat_data['info']
        
        # Calc stats
        total_courses = sum(len(r['courses']) for r in routes_list)
        total_classes = 0
        for r in routes_list:
            for c in r['courses']:
                for m in c.get('modules', []):
                    total_classes += len(m.get('classes', []))

        final_categories.append({
            'id': c_info.get('id', cat_id),
            'name': c_info.get('name', 'Categoría'),
            'icon': c_info.get('icon', '📂'),
            'description': c_info.get('description', ''),
            'folderName': c_info.get('folderName', ''),
            'routes': routes_list,
            'routeCount': len(routes_list),
            'courseCount': total_courses,
            'classCount': total_classes
        })

    # Stats for cache
    total_routes = sum(len(cat['routes']) for cat in final_categories)
    total_courses = sum(cat['courseCount'] for cat in final_categories)
    total_classes = sum(cat['classCount'] for cat in final_categories)
    
    print(f"     ✓ Drive: {total_courses} cursos, {total_classes} clases mapeadas.")

    return {
        'categories': final_categories,
        'stats': {
            'totalCategories': len(final_categories),
...
def scan_all_courses():
    ...
    # PORTABLE MODE: If DRIVE_ROOT_ID is set and no local override, use Drive.
    # We can check if local COURSES_PATH exists. If not, use Drive.
    # Or just always use Drive if configured for portability.
    
    # Check if local path exists and has content
    local_exists = os.path.exists(COURSES_PATH) and os.path.isdir(COURSES_PATH)
    try:
        if local_exists and not os.listdir(COURSES_PATH):
             local_exists = False
    except:
        local_exists = False
        
    # FORCE DRIVE MODE FOR PORTABILITY (Requested by user)
    # The user wants "portable app... that works 100% with drive".
    # So we should prioritize Drive scanning.
    
    print(f"🔄 Modo Portátil: Escaneando desde Google Drive...")
    return scan_drive_structure(DRIVE_ROOT_ID)

    # Legacy local scanning (Disabled/Fallback)
    categories = []
